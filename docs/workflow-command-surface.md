...
| `CMD-STAGE-ADVANCE` | Advance To Next Stage | `load_or_create_artifact` | Approved checkpoint and run record for non-PLAN stages only. | No | May select the next stage after checkpoint approval; stop on unapproved current stage, PLAN stage, stale artifact version, missing matching checkpoint, active/pending confirmation need, or open route. | Update `run.md` to `next_stage` and set `current_stage` to the next stage; `CMD-GATE-ENTRY` performs the later `active_stage_draft` transition. |
...
| `CMD-REVIEW-CHECKPOINT` | Run Checkpoint Review | `run_checkpoint_review` | Artifact with Quality Gate `ready`. | No | Current CLI MVP is marker-only; stop if Quality Gate is not ready, the artifact is stale, or the marker cannot be written. | Write `reviews/<stage>-checkpoint-review-vN.md`; result `checkpoint_review` ready for human decision. |
...
| `CMD-CHECKPOINT-DECIDE` | Approve Checkpoint | `approve_checkpoint` | Ready artifact, checkpoint-review marker for approvals, required user confirmations. | Required for approval | May approve only when no unresolved required change or route remains; may request changes without approval. Stop approved decisions if the marker, confirmation, or required version-matched subagent review is missing. | Update checkpoint section and `run.md`; result `approved` or `changes_requested` in the current CLI. |
...
`CMD-TIER-ESTIMATE` is a stateless read that may run whenever its text input exists. `CMD-TIER-LOCK` is a write accepted only in `active_stage_draft`; once a run leaves the initial draft state, the operator must reopen or repair the run instead of silently replacing the lock. `CMD-TIER-ESCALATE` is allowed in nonterminal states before checkpoint approval (`active_stage_draft`, `entry_gate_failed`, `quality_gate_failed`, `ready_for_checkpoint_review`, `checkpoint_review`, `checkpoint_changes_requested`, `upstream_gap_routing`, `next_stage`). It is refused in `not_started`, `checkpoint_approved`, `closed_at_plan_checkpoint`, and inconsistent or unknown states.
...
| `quality_gate_failed` | `CMD-STAGE-PRODUCE`, `CMD-STAGE-UPDATE`, `CMD-STAGE-READY`, `CMD-CONFIRM-RECORD`, `CMD-CONFIRM-REJECT`, `CMD-SUBAGENT-DISPATCH`, `CMD-SUBAGENT-MERGE`, `CMD-GAP-RECORD`, `CMD-GAP-ROUTE` | Stop checkpoint review and checkpoint decision until the artifact is repaired, returned to `active_stage_draft`, marked ready, and Quality Gate returns `ready`. |
| `ready_for_checkpoint_review` | `CMD-REVIEW-CHECKPOINT`, `CMD-SUBAGENT-REVIEW`, `CMD-GAP-RECORD` | Stop checkpoint decision until checkpoint review is opened and the version-matched marker exists. |
| `checkpoint_review` | `CMD-REVIEW-CHECKPOINT` or `CMD-SUBAGENT-REVIEW` only for required review sources registered before review began, `CMD-REVIEW-MERGE`, `CMD-CONFIRM-RECORD`, `CMD-CONFIRM-REJECT`, `CMD-CONFIRM-LINK`, `CMD-CHECKPOINT-DECIDE`, `CMD-CHECKPOINT-BUNDLE` only for eligible no-modifier tiers with each bundled stage's Quality Gate `ready` and review preconditions satisfied, `CMD-GAP-RECORD`, `CMD-GAP-ROUTE`, `CMD-TIER-ESCALATE` | Stop new unregistered review dispatch; allow confirmation evidence to be recorded while waiting for checkpoint decision. Stop `approved` decisions when the checkpoint-review marker, required confirmation, required subagent review, or route resolution is missing; allow `changes_requested` to record a non-approval result. Stop `CMD-CHECKPOINT-BUNDLE` when any modifier is present or the requested stage set is not bundle-eligible. |
| `checkpoint_changes_requested` | `CMD-STAGE-PRODUCE`, `CMD-STAGE-UPDATE`, `CMD-CONFIRM-RECORD`, `CMD-CONFIRM-REJECT`, `CMD-GAP-RECORD`, `CMD-GAP-ROUTE` | Stop checkpoint decision until requested changes produce a repaired draft/new artifact version, return the run to `active_stage_draft`, the artifact is marked ready, and Quality Gate reruns. |
