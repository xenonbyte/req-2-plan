...
PLAN 检查点关闭的 run，用 `r2p-reopen --from <id> --stage <拥有阶段>` 重开，并触发同样的
下游级联——它不是单阶段编辑。**状态名与合法转移集合一律以代码 `models.py`（`RunStatus`、
`ALLOWED_TRANSITIONS`）为准。**
