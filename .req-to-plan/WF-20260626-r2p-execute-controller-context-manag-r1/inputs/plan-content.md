...
- [ ] Cover the `tools/r2p-task-brief` wrapper (from PLAN-TASK-002) two ways: (1) structural — assert it is discovered/installed and its body delegates to `agent_shortcuts task-brief` (grep), per the existing install wrapper-coverage test; (2) end-to-end — subprocess-run the wrapper and assert identical JSON, file, and exit code as the CLI, making `python3` resolve to a dependency-complete interpreter by prepending to PATH a temp dir whose `python3` symlinks to `sys.executable` (never a hardcoded `.venv/bin`, which may be absent on CI); the wrapper sets its own PYTHONPATH.
...
- PLAN-TASK-2 (Phase 1, independently shippable with PLAN-TASK-1) — implement the `task-brief` shortcut, the `tools/r2p-task-brief` wrapper, and shortcut/wrapper equivalence tests. Spec References: SPEC-SURFACE-001. Carries SCOPE-IN-001.
- PLAN-TASK-3 (Phase 2) — add the FR-CM1 through FR-CM4 prose to both r2p-execute surfaces and extend `tests/test_docs_consistency.py` token coverage. Spec References: SPEC-PROSE-001, SPEC-PROSE-002, SPEC-PROSE-003, SPEC-PROSE-004, SPEC-GUARD-001. Carries SCOPE-IN-002, SCOPE-IN-003, SCOPE-IN-004, SCOPE-IN-005, SCOPE-IN-006.
...
| SPEC-SURFACE-001 | DES-CLI-001, SCOPE-IN-001 | open — consumed by PLAN-TASK-2 |
| SPEC-PROSE-001 | DES-PROSE-001, SCOPE-IN-002 | open — consumed by PLAN-TASK-3 |
| SPEC-PROSE-002 | DES-PROSE-001, SCOPE-IN-003 | open — consumed by PLAN-TASK-3 |
| SPEC-PROSE-003 | DES-PROSE-001, SCOPE-IN-004 | open — consumed by PLAN-TASK-3 |
| SPEC-PROSE-004 | DES-PROSE-001, SCOPE-IN-005 | open — consumed by PLAN-TASK-3 |
