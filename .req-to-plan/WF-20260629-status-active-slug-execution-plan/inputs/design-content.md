...
- PLN-2 / PLN-3 / PLN-5 shape — chosen: seed `Out-of-Task` and `Future Task Ownership` as optional field lines inside the PLAN-TASK body with gate-clean illustrative values (referencing the `PLAN-TASK-NNN` guidance form, never added to `PLAN_TASK_FIELDS`); keep the required `Verification` field a guided `<!-- fill in: ... -->` placeholder but enrich its example into the three-part contract; seed PLN-5 granularity guidance as a concise HTML comment under `## Tasks`.
