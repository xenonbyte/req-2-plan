...
r2p_version: 2
r2p_status: approved
r2p_created_at: 2026-08-31T16:26:06.934883+00:00
r2p_updated_at: 2026-08-31T16:33:36.590830+00:00
---
...
- This is a delta PLAN reopened from the original execution after its Task 9 exposed a PLAN file-authority defect. Current source HEAD `ac3233cd9782c96a665e0f56e43fc17c5d82187f` already contains the reviewed original Task 1–8 implementation; the reopened run must not replay those tasks or manufacture no-op commits.
...
- Do not replay original Task 1–8, mutate the three archived sample directories, or use old execution reports as role inputs. Only the validator's no-follow read access to those exact paths is authorized; current HEAD and fresh deterministic tests are the operational baseline.
...
- [ ] [ADDRESSED] Treat current HEAD as the implementation baseline for the already reviewed original Task 1–8. Before source mutation, confirm only `.req-to-plan/` is dirty and this task is the sole reopened PLAN task; run the exact sample gate from Execution Readiness, require accepted evidence in this run without rereading samples, confirm HEAD is unchanged, then require prerequisite-check v1 `satisfied=true` for task 1.
...
Verification:
1. Run `/opt/homebrew/opt/python@3.14/bin/python3.14 -E /Users/xubo/x-skills/req-to-plan/tools/workflow_cli/__main__.py tools.workflow_cli --base-path /Users/xubo/x-skills/req-to-plan execution-prerequisite-check --work-id WF-20260829-r2p-execute-token-phase-r2p-r2 --task 1 --require-version 1`; require `satisfied=true` before dispatch.
...
若原始执行在 Task 009 因 PLAN `Files` 权限缺口停止并从 SPEC/PLAN 重开，reopened run 以重开时的 clean source HEAD 作为 operational baseline：已 reviewed-complete 的原 Task 001–008 不得重放、不得生成 no-op commits，也不得复制旧 execution ledger。新 PLAN 只形成一个从 001 重新编号的 modify-only delta task，合并原 Task 009 integration paths 与完成 fast role topology 所必需的 `execution_metrics.py`/direct tests；该 task 承担新的 Phase acceptance。这个 delta 例外不改变未来由 continue surfaces 生成的 `2 / 4 / 1 / 2` 布局规则。
...
本次 reopened run 只规划一个从 `PLAN-TASK-001` 重新编号的 operation-homogeneous `modify` delta；当前 clean HEAD `ac3233cd9782c96a665e0f56e43fc17c5d82187f` 是原执行 Task 001–008 reviewed-complete 后的 source baseline。PLAN 不得重放这些任务、复制旧 execution ledger或创建 no-op commits。
...
3. 第一条 semantic Steps line 为 `Prerequisite: none`；第一条 Verification 运行 reopened work ID 的 checker v1。实现本身升级 checker 到 v2并同步五个 PLAN-author surfaces。
...
| This ID | Upstream | Status |
|---|---|---|
| Spec headings above | Approved DESIGN v8; execution-reopen source baseline at ac3233c | [ADDRESSED] Behavior contracts remain intact; PLAN handoff now prevents replay of reviewed-complete work and gives the remaining profile-aware metrics integration explicit file authority. |
<!-- /r2p-read-only -->
