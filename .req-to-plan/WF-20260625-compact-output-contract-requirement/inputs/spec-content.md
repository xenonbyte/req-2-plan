...
- shown count;
- total count;
- recovery path when the caller successfully produced one.
...
- `tools/workflow_cli/cli.py`
  - Add a CLI-local recovery-log helper that receives `run_dir`, deterministic
    filename, and full item list; returns the relative or absolute path only on
    successful write.
...
| Source | Result |
|---|---|
| External library docs | N/A - no external dependencies or external API behavior are required for this change. |
| Local code evidence | Checked via CodeGraph for `output.py`, `cli.py`, `workspace.py`, `state.py`, and gate/test surfaces. |
