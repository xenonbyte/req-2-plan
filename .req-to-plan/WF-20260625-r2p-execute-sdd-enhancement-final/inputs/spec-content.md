...
|---|---|---|---|
| missing-file fail | `tests/test_gates.py` | no marker → fail, exit 3 | SPEC-GATE-001a |
| symlink fail | `tests/test_gates.py` | symlink target → fail | SPEC-GATE-001b |
| non-regular (FIFO) fail | `tests/test_gates.py` | FIFO → fail, no block | SPEC-GATE-001c |
| no-verdict-line fail | `tests/test_gates.py` | regular file, no `Verdict:` → fail | SPEC-GATE-001d |
| unsupported value fail | `tests/test_gates.py` | `Verdict: Maybe` → fail | SPEC-GATE-001e |
| changes-requested fail | `tests/test_gates.py` | final `Verdict: Changes Requested` → fail | SPEC-GATE-001f |
| approved pass | `tests/test_gates.py` | final `Verdict: Approved` → pass, exit 0 | SPEC-GATE-001g |
| last-of-many: CR then Approved | `tests/test_gates.py` | passes | SPEC-GATE-001h |
| last-of-many: Approved then CR | `tests/test_gates.py` | fails | SPEC-GATE-001h |
| fenced verdict ignored | `tests/test_gates.py` | `Verdict:` only inside ``` → not counted (→ fail d) | SPEC-GATE-001i |
