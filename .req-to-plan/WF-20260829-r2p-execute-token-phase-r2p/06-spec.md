...
r2p_version: 7
r2p_status: approved
r2p_created_at: 2026-08-29T14:37:38.087849+00:00
r2p_updated_at: 2026-08-29T16:13:48.622356+00:00
---
...
Phase 0 必须落地只读 internal command `execution-samples-validate`，公开给 controller 的参数恰为重复三次的 `--sample-dir <absolute-archived-run-dir>`；没有默认扫描、glob、相对路径、第四份样本或隐式选择。Phase 3 controller 只接受用户明确提供的三个 absolute paths，并在派发 `PLAN-TASK-008` 及任何 Phase 3 source mutation 前执行该命令。参数个数不是三、canonical path 重复或证据不足时输出 `BLOCKED: representative_metrics_missing` 并 exit 3；未知 flag/语法错误 exit 2。命令本身只写 stdout/stderr，不修改样本、当前 run 或源码；controller 仅可把 JSON stdout 重定向到当前 run 已忽略的 `execution/phase-3-sample-evidence.json`。
...
任一条件失败时，controller 不派发 Phase 3 role；`PLAN-TASK-008/009` 保持 `[ ]`，source worktree/HEAD 必须与 Phase 3 BASE 相同。证据通过后，Task 008 只消费并引用 `execution/phase-3-sample-evidence.json`，不得二次读取样本目录；其 report 直接呈现每个 Sample 的 identity/header/verdict/coverage/rules 与全部 measured aggregates。跨 `direct_acs`/`semantic_view` 只比较各自 context totals，不能把不同 byte kind 直接相减为 Token 收益；TokenTotals unavailable 时固定写 `token comparison: unavailable`。
...
本 PLAN 的布局和编号固定为 `2 / 4 / 1 / 2`：Phase 0 = 001 metrics core create → 002 integration modify；Phase 1 = 003 context core create → 004 internal CLI modify → 005 wrapper create/smoke → 006 surface adoption modify；Phase 2 = 007 modify；Phase 3 = 008 profile core create → 009 integration modify。依赖不新增 field。每个 task 的 `Steps` 第一条 semantic line exact 为 `Prerequisite: none` 或 `Prerequisite: PLAN-TASK-NNN`；declared edges 仅为 001→002、003→004→005→006、008→009，001/003/007/008 使用 none。跨 Phase 顺序只由 PLAN 编号、最低 actionable task selector 与上一 Phase acceptance 控制，不进入 rollback graph。
...
- Phase 2 首次把上述五面写为 prerequisite checker semantics v1；Phase 3 Task 009 在同一 patch 把这五面与 OpenCode-derived test 升级为 v2。两次都是现有文件 modify，不新增生成面或 capability auto-detection。
...
| SPEC-GRANULARITY-004 / SPEC-PARITY-008 | Task001/002 legacy bootstrap matrix；implementation×requested `1/1,1/2,2/1,2/2` semantics/exit/JSON fields；Task007五面静态v1、Task009同五面静态v2及旧PLAN兼容；`2/4/1/2` operation-homogeneous layout、Steps-only exact grammar、group-only edge/rollback、OpenCode derived install + Gemini description；PLAN fields/schema/trace tests unchanged。 |
...
## PLAN Handoff
PLAN 必须形成四个顺序 phase-level cohesive slices，并按现有 R19 file gate 精确展开为九个 operation-homogeneous tasks。每个 task 的 `Files` 只含同一 create/modify operation 的完整实现与直接测试；同组最后一个 integration/adoption task 覆盖必要同步 surface 和 Phase acceptance。`Steps` 第一条使用 SPEC-GRANULARITY-004 的 canonical prerequisite，且引用下列完整 SPEC 集合：

1. **Phase 0 / PLAN-TASK-001–002 — cadence/zero-history/metrics foundation**：001 create metrics parser/transaction/classifier/sample-validator 和 prerequisite-checker v1 core 与 direct tests；002 modify CLI/shortcut/execute surfaces/tests/docs，注册 `execution-prerequisite-check` v1并接入 SPEC-VERIFY-001、SPEC-ROLE-002、SPEC-METRICS-009 和相关 SPEC-PARITY-008，承担 Phase acceptance。Task 001/002 分别使用 SPEC-GRANULARITY-004 legacy bootstrap preflight；Task 002 reviewed complete 后、Task 003 dispatch 前运行当前 self-host exact bootstrap command。
2. **Phase 1 / PLAN-TASK-003–006 — context view/compact audit**：003 create context core/direct tests；004 modify internal CLI/tests；005 create wrapper/smoke test（此时 CLI target 已存在）；006 modify agent surfaces/install/docs/consistency tests并承担 Phase acceptance。实现 SPEC-CONTEXT-010、SPEC-CONTEXT-011、SPEC-REPORT-012 与相关 parity。
3. **Phase 2 / PLAN-TASK-007 — PLAN task granularity**：单个 modify task 实现 SPEC-GRANULARITY-004 与 continue 部分的 SPEC-PARITY-008；在 Phase 3 adoption 前明确只要求 prerequisite-checker v1/strict-compatible preflight，不修改 PLAN schema/gates。
4. **Phase 3 / PLAN-TASK-008–009 — strict/fast profile**：Task 008 dispatch/source mutation 前，用用户提供的三个 absolute sample dirs 执行 SPEC-SAMPLE-003 exact validator；通过后 008 create profile/ledger core/tests，009 modify CLI/shortcut/execute surfaces、Phase 2 五个 continue/PLAN-author surfaces、tests/docs，将 prerequisite implementation和生成要求同时升级到 v2并承担 Phase acceptance，实现 SPEC-PROFILE-013、SPEC-LEDGER-014、SPEC-FAST-005、SPEC-RESUME-006、SPEC-FINAL-007 与 profile-related parity。证据失败时 008/009 均未完成且 source/HEAD 保持 Phase 3 BASE。
...
|---|---|---|
| Spec headings above | Approved DESIGN v8 | [ADDRESSED] Every chosen design decision is encoded as behavior, API/data, error, recovery, observability, and test contracts. |
