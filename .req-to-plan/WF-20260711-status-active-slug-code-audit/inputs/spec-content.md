...
|---|---|---|
| N/A | — | Stdlib-only change set (`re` lookarounds, `os`/`pathlib` already in use); no external library, framework, or API surface is touched, so no external documentation lookup applies. |
